...
public class TakeBlipMB {
	...
	/*
	 * Olá {{1}}, o lead {{2}} do cliente {{3}} acabou de chegar, favor entrar em
	 * contato.
	 */

	// contrato_pre_aprovado
	/*
	 * Olá {{1}}, ótima notícia! 😆 Cliente {{2}} do Contrato {{3}} foi pré aprovado
	 * com taxa de {{4}}% + IPCA e prazo de {{5}} meses
	 */

	// contrato_recebido_paju
	/*
	 * Olá {{1}}, tudo certo? Já recebemos o paju do Cliente {{2}} do Contrato
	 * {{3}}. 😉
	 */

	// contrato_recebido_laudo
	/*
	 * Olá {{1}}, tudo certo? Já recebemos o laudo do Cliente {{2}} do Contrato
	 * {{3}}. 😉
	 */

	// contrato_recebido_laudo_paju
	/*
	 * Olá {{1}}, tudo certo? Já recebemos o laudo e paju do Cliente {{2}} do
	 * Contrato {{3}}, o próximo passo é o comitê 😉
	 */

	// ag_comentarios_juridico
	/*
	 * Olá {{1}}, contrato {{2}} do cliente {{3}} está aguardando seu comentário
	 * jurídico
	 */

	// comentado_juridico_interno
	/*
	 * Olá {{1}}, contrato {{2}} do cliente {{2}} já foi comentado pelo jurídico
	 * interno
	 */

	// ag_validacao_documentos
	/*
	 * Olá {{1}}, contrato {{2}} do cliente {{3}} está aguardando sua validação de
	 * documentos
	 */

	// aprovado_comite_ag_doc
	/*
	 * Olá {{1}}, contrato {{2}} do cliente {{3}} foi aprovado pelo comitê e está
	 * pronto para fazer a análise dos documentos
	 */

	// aprovado_comite_ag_ccb
	/*
	 * Olá {{1}}, contrato {{2}} do cliente {{3}} foi aprovado pelo comitê e está
	 * pronto para fazer a CCB
	 */

	// contrato_pronto_para_assinatura
	/*
	 * Olá {{1}}, estamos quase lá! Contrato do Cliente {{2}} do Contrato {{3}} já
	 ...
	 * Olá {{1}}, tudo pronto!
	 * 
	 * Contrato do Cliente {{2}} do Contrato {{3}} já foi dado entrada no Cartório
	 * de Registro.
	 * 
	 ...
	 * Olá {{1}}!!!
	 * 
	 * Parabéns pela pré-aprovação do seu Crédito com Imóvel em Garantia aqui na
	 * Galleria Bank 🤑
	 * 
	 * Nós, junto com o seu consultor financeiro, queremos fazer parte dos seus
	 * projetos.
	 * 
	 * Logo mais entraremos em contato com você para a agendar o Laudo de Avaliação
	 * do seu imóvel.
	 * 
	 ...
	 * projetos.
	 * 
	 * Logo mais, a empresa Galache estará entrando em contato com você para a
	 * agendar o Laudo de Avaliação do seu imóvel.
	 * 
	 * Conte sempre com a gente
	 * 
	 * Até mais.
	...
	/*
	 * Olá {{1}}, aqui é da Galleria Bank. Confirmando a vistoria do imóvel para
	 * {{2}} ({{3}}) às {{4}}. A visita será feita por {{5}}. Até logo
	 */
	...
	/*
	 * Olá {{1}}, contrato {{2}} do cliente {{3}} precisa de atualização referente
	 * ao cartório. Próxima notificação marcada para {{4}}
	 */
	 ...
	 * "to":"5519991653911@wa.gw.msging.net", "type":"application/json", "content":{
	 * "type":"template", "template":{
	 * "namespace":"37de7635_839c_4792_92a6_5d40dc299b2a",
	 * "name":"sample_issue_resolution", "language":{ "code":"pt_BR",
	 * "policy":"deterministic" }, "components":[ { "type": "body", "parameters": [
	 * { "type": "text", "text": "Hermes Jr" } ] } ] } } }
	 */
	...
	public void senderWhatsAppMessage(JSONObject jsonWhatsApp) {
		...
		try {
			...
			/**
			 * TODO SALVAR NO BANCO O ID DE TODAS AS TRANSFERENCIAS USAR ESTE ID PARA BUSCAR
			 * O STATUS DA TRANSFERENCIA https://api.iugu.com/v1/withdraw_requests/id"
			 */
			if (myURLConnection.getResponseCode() != HTTP_COD_SUCESSO) {
				context.addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_INFO,
								"Take Blip: Falha ao enviar a mensagem WhatsApp (Cod: "
	...
	public void sendWhatsAppNotificaResponsavel(Responsavel responsavel, String nomeTemplateMensagem, String frase1,
			String frase2) {
	...
	public void sendWhatsAppMessage(Responsavel responsavel, String nomeTemplateMensagem, String nomeDoCliente,
			String numeroDoContrato, String taxaJuros, String prazo) {
		JSONObject jsonWhatsApp = new JSONObject();
		jsonWhatsApp.put("id", generateUUID());
		...
		if (!CommonsUtil.semValor(responsavel.getDonoResponsavel())) {
			...
			// envia a mensagem para o gerente do gerente
			if (!CommonsUtil.semValor(responsavel.getDonoResponsavel().getDonoResponsavel())
					&& !CommonsUtil.mesmoValor(responsavel.getDonoResponsavel().getDonoResponsavel().getId(),
							responsavel.getDonoResponsavel().getId())) {
				Responsavel rGerenteGerente = responsavel.getDonoResponsavel().getDonoResponsavel();
				// rAssistente = rDao.findById(SiscoatConstants.ASSISTENTE_ISA);
				this.sendWhatsAppMessage(rGerenteGerente, nomeTemplateMensagem, nomeDoCliente, numeroDoContrato,
						taxaJuros, prazo);
			}
		}
	}
	...
	public void sendWhatsAppMessageCobranca(Responsavel responsavel, String nomeTemplateMensagem,
			Date dataVencimento, String numeroDoContrato, String nomeCliente) {
		JSONObject jsonWhatsApp = new JSONObject();
		jsonWhatsApp.put("id", generateUUID());
		...
		jsonWhatsAppParameter = new JSONObject();
		jsonWhatsAppParameter.put("type", "text");
		jsonWhatsAppParameter.put("text", CommonsUtil.formataData(dataVencimento, "dd/MM/yyyy"));
		jsonWhatsAppParameters.put(jsonWhatsAppParameter);
